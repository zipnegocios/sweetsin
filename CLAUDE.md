...
- **Apps mobile y staff (Fase 7):** `apps/mobile` es un cliente Expo (iOS/Android) que se comunica con Route Handlers REST de `apps/web` en `/api/mobile/**` — sin backend separado. Autenticación por PIN de 6 dígitos (`User.pinHash`, hash bcrypt/argon2, con rate limiting y lockout temporal vía `User.failedPinAttempts`/`pinLockedUntil`), JWT de sesión de TTL corto revocable vía `is_active` (mismo patrón que web Auth.js de Fase 4), y validación de integridad del cliente (Play Integrity/DeviceCheck o HMAC de build). Páginas nuevas `/dispatch` y `/delivery` (accesibles solo en web) sirven como superficies de staff para gestionar órdenes — separadas de `/admin/orders` para permitir flujos y datos distintos (cola de despacho vs. historial). Notificaciones push registradas en tabla `push_logs` (separada deliberadamente de `email_logs` de Fase 6 por transporte diferente y audiencia distinta: push es para dispositivos, email para usuarios invitados/clientes web) — ambas alimentan un audit trail centralizado pero sin compartir esquema.
- **`notifyNewOrderInQueue` sin conectar (Fase 7, fix wave final):** existe en `StaffNotificationPort` y en `ExpoNotificationAdapter`, y está cubierto por tests unitarios, pero ningún caso de uso de producción lo invoca todavía — el candidato natural es `confirmOrderPayment` (`packages/domain/src/orders/use-cases.ts`), en el momento en que una orden paga entra a `received`. Conectarlo queda fuera de alcance de esta fase; es trabajo futuro explícito, no un olvido.

## Product

Sitio público bilingüe de Sweet Sin (inglés en la raíz `/`, español en `/es`): hero, catálogo de 16 postres reales (paradas y precios desde Postgres), historia de marca, ubicaciones activas del trailer con mapa en vivo, un formulario de cotización de eventos que persiste la solicitud real, y carrito + checkout de invitado (pickup/self-delivery, pago vía WhatsApp funcional de punta a punta; pago con tarjeta scaffoldeado con Stripe, pendiente de credenciales reales). Cuenta de cliente (login/registro/historial de pedidos) y panel admin completo (gestión de órdenes de Fase 4; paradas del trailer con mapa interactivo y control de stock, reservas de eventos con flujo de cotización a confirmación, y una vista de calendario combinando ambas, de Fase 5) ya funcionan de punta a punta, protegidos con Auth.js.
...
- `/admin/orders` (Fase 4) y la máquina de estados de despachador/delivery (Fase 7) no están coordinadas — `updateOrderStatusAction` (`apps/web/src/app/actions/admin-orders.ts`) escribe cualquier `fulfillmentStatus` sin guardas de transición ni manejo de `assignedDeliveryUserId`. Un admin puede, por ejemplo, setear `out_for_delivery` manualmente sin que haya `assignedDeliveryUserId`, dejando la orden sin nadie que pueda marcarla entregada desde `apps/mobile`. Costura aceptada, no una regresión — el plan de Fase 7 nunca puso `/admin/orders` en alcance. Si en el futuro se decide coordinar ambos flujos, revisar `updateOrderStatusAction` para que respete las mismas transiciones/limpieza de `assignedDeliveryUserId` que `packages/domain/src/orders/use-cases.ts`.
